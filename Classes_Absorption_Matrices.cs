...
namespace Pachyderm_Acoustic
{
    namespace AbsorptionModels
    {
        ...
        {
            //class StateMatrix_Fluid : SparseMatrix
            //{
            //    public StateMatrix_Fluid(Complex p, Complex u)
            //        : base(2, 1)
            //    {
            //        this[0, 0] = p;
            //        this[0, 1] = u;
            ...
            //class StateMatrix_Porous : SparseMatrix
            //{
            //    public StateMatrix_Porous(Complex V1s, Complex V3s, Complex v3f, Complex sig33s, Complex sig13s, Complex sig33f)
            //        : base(6, 1)
            //    {
            //        this[0, 0] = V1s;
            //        this[0, 1] = V3s;
            ...
            //}
                ...
                public static SparseMatrix PorousMatrix(bool Rigid, double d, Complex k, Complex sin_theta, double freq, double porosity, double tortuosity, double YoungsModulus, double PoissonRatio, double Viscous_Characteristic_Length, double flow_resistivity, double FrameDensity, double Thermal_Permeability_0, double AmbientMeanPressure)
                {
                    ...
                    public static SparseMatrix GammaH_P(Complex kt, double w, double h, double ShearModulus, Complex P, Complex Q, Complex R, Complex k13, Complex k23, Complex k33, Complex Mu1, Complex Mu2, Complex Mu3)
                    {
                        ...
                        M[0, 1] = -Complex.ImaginaryOne * k13 * Complex.Sin(k13 * h);
                        M[0, 2] = -Complex.ImaginaryOne * k13 * Mu1 * Complex.Sin(k13 * h);
                        M[0, 3] = -D1 * Complex.Cos(k13 * h);
                        M[0, 4] = 2 * Complex.ImaginaryOne * ShearModulus * kt * k13 * Complex.Sin(k13 * h);
                        M[0, 5] = -E1 * Complex.Cos(k13 * h);
                        M[1, 0] = -Complex.ImaginaryOne * w * kt * Complex.Sin(k13 * h);
                        M[1, 1] = w * k13 * Complex.Sin(k13 * h);
                        M[1, 2] = w * k13 * Mu1 * Complex.Sin(k13 * h);
                        M[1, 3] = Complex.ImaginaryOne * D1 * Complex.Sin(k13 * h);
                        M[1, 4] = -2 * ShearModulus * kt * k13 * Complex.Cos(k13 * h);
                        M[1, 5] = Complex.ImaginaryOne * E1 * Complex.Sin(k13 * h);

                        M[2, 0] = w * kt * Complex.Cos(k23 * h);
                        M[2, 1] = -Complex.ImaginaryOne * k23 * Complex.Sin(k23 * h);
                        M[2, 2] = -Complex.ImaginaryOne * k23 * Mu2 * Complex.Sin(k23 * h);
                        M[2, 3] = -D2 * Complex.Cos(k23 * h);
                        M[2, 4] = 2 * Complex.ImaginaryOne * ShearModulus * kt * k23 * Complex.Sin(k23 * h);
                        M[2, 5] = -E2 * Complex.Cos(k23 * h);
                        M[3, 0] = -Complex.ImaginaryOne * w * kt * Complex.Sin(k23 * h);
                        M[3, 1] = w * k23 * Complex.Sin(k23 * h);
                        M[3, 2] = w * k23 * Mu2 * Complex.Sin(k23 * h);
                        M[3, 3] = Complex.ImaginaryOne * D1 * Complex.Sin(k23 * h);
                        M[3, 4] = -2 * ShearModulus * kt * k23 * Complex.Cos(k23 * h);
                        M[3, 5] = Complex.ImaginaryOne * E2 * Complex.Sin(k23 * h);

                        M[4, 0] = Complex.ImaginaryOne * k33 * Complex.Sin(k33 * h);
                        M[4, 1] = w * kt * Complex.Cos(k33 * h);
                        M[4, 2] = w * kt * Mu3 * Complex.Cos(k33 * h);
                        M[4, 3] = 2 * Complex.ImaginaryOne * ShearModulus * k33 * kt * Complex.Sin(k33 * h);
                        M[4, 4] = ShearModulus * (k33 * k33 - kt * kt) * Complex.Cos(k33 * h);
                        M[4, 5] = 0;
                        M[5, 0] = -w * k33 * Complex.Cos(k33 * h);
                        M[5, 1] = -Complex.ImaginaryOne * w * kt * Complex.Sin(k33 * h);
                        M[5, 2] = -Complex.ImaginaryOne * 2 * kt * Mu3 * Complex.Sin(k33 * h);
                        M[5, 3] = -2 * ShearModulus * k33 * kt * Complex.Cos(k33 * h);
                        M[5, 4] = -Complex.ImaginaryOne * ShearModulus * (k33 * k33 - kt * kt) * Complex.Sin(k33 * h);
                        M[5, 5] = 0;
                    ...
                    public static SparseMatrix FluidMatrix(double h, Complex kz, double freq, double rho)
                    {
                        SparseMatrix M = new SparseMatrix(2,2);
                        Complex kzh = kz * h;
                        Complex sinkzh = Complex.Sin(kzh);
                        Complex coskzh = Complex.Cos(kzh);
                        double pr2 = Utilities.Numerics.PiX2 * freq * rho;
                        
                        M[0, 0] = coskzh;
                        M[0, 1] = (Complex.ImaginaryOne * kz / pr2) * sinkzh;
                        M[1, 0] = (Complex.ImaginaryOne * pr2 / kz) * sinkzh;
                        M[1, 1] = coskzh;
                    ...
                    public static SparseMatrix Solid_Matrix(Complex ksolid, Complex kprev, Complex SinTheta, double h, double freq, double density, double PoissonRatio, double ModulusElasticity)
                    {
                        ...
                        MH[0, 0] = wk1 * cosk13h;
                        MH[0, 1] = -Complex.ImaginaryOne * wk13 * cosk13h;
                        MH[0, 2] = -D1 * cosk13h;
                        MH[0, 3] = Complex.ImaginaryOne * D2k13 * sink13h;
                        MH[1, 0] = -Complex.ImaginaryOne * wk1 * sink13h;
                        MH[1, 1] = wk13 * cosk13h;
                        MH[1, 2] = Complex.ImaginaryOne * D1 * sink13h;
                        MH[1, 3] = -D2k13 * cosk13h;
                        MH[2, 0] = Complex.ImaginaryOne * wk33 * sink33h;
                        MH[2, 1] = wk1 * cosk33h;
                        MH[2, 2] = Complex.ImaginaryOne * D2k33 * sink33h;
                        MH[2, 3] = D1 * cosk33h;
                        MH[3, 0] = -wk33 * cosk33h;
                        MH[3, 1] = -Complex.ImaginaryOne * wk1 * sink33h;
                        MH[3, 2] = -D2k33 * cosk33h;
                        MH[3, 3] = -Complex.ImaginaryOne * D1 * sink33h;

                        SparseMatrix M0 = new SparseMatrix(4, 4);
                        M0[0, 0] = 2 * k1 / (w * delta23);
                        M0[0, 3] = (k33 * k33 - k1 * k1) / (w * k33 * delta23);
                        M0[1, 1] = (k33 * k33 - k1 * k1) / (w * k13 * delta23);
                        M0[1, 2] = (k1) / (w * delta23);
                        M0[2, 0] = -1 / (LameMu * delta23);
                        M0[2, 3] = -k1 / (LameMu * k33 * delta23);
                        M0[3, 1] = -k1 / (LameMu * k13 * delta23);
                        M0[3, 2] = 1 / (LameMu * delta23);
            ...
            //public static SparseMatrix GlobalTransfer()
            //{
            //    MathNet.Numerics.LinearAlgebra.Matrix<SparseMatrix> D0 = new
            //}
        }
    }
}