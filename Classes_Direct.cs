    ...
    public class Direct_Sound:Simulation_Type
    {
            ...
            double[] Pn = new double[P[Rec_ID].Length];
                if (Dir_Rec_Pos[Rec_ID].GetLength(1) > 1)
                {
                    for (int i = 0; i < Pdir[Rec_ID].Length; i++)
                    {
                        ...
                        double[] P_temp = Audio.Pach_SP.Linear_Phase_Signal(Eo, SampleFreq, 2048, 0);
        ...
        public void Create_Pressure()
        {
            ...
            for(int i = 0; i < Receiver.Count; i++)
            {
                ...
                P[i] = new double[ETC[0].Length + 2048];
                Pdir[i] = new double[6][];
                for (int j = 0; j < 6; j++) Pdir[i][j] = new double[ETC[0].Length + 2048];
                    for (int t = 0; t < ETC[0].Length; t++)
                    {
                        ...
                        double[] Pmin = Audio.Pach_SP.Linear_Phase_Signal(new double[8] { ETC[0][0], ETC[1][0], ETC[2][0], ETC[3][0], ETC[4][0], ETC[5][0], ETC[6][0], ETC[7][0] }, 44100, 2048, 0);
