    ...
    namespace Numeric
    {
        namespace TimeDomain
        {
            public partial class Acoustic_Compact_FDTD : Simulation_Type
            {
                ...
                Acoustic_Compact_FDTD.Node[][][] PFrame;            //pressure scalar field initialisation
                ...
                public void Build_FVM13( ref int xDim, ref int yDim, ref int zDim)
                {
                    ...
                    for (int x = 0; x < Math.Floor((double)xDim); x++)
                    {
                        int mod = x % 2;
                        PFrame[x] = new Node[yDim][];
                        Random Rnd = new Random(x);
                        for (int y = 0; y < Math.Floor((double)yDim/2) + mod; y++)
                        {
                            PFrame[x][y] = new Node[zDim];
                            for (int z = 0; z < Math.Floor((double)zDim/2) + mod; z++)
                            {
                                List<double[]> abs;
                                List<Bound_Node.Boundary> BDir;
                                Point Loc = new Point(Bounds.Min_PT.x + (((double)x + 0.5) * dx), Bounds.Min_PT.y + (((double)y + (1.5 - mod)) * dy), Bounds.Min_PT.z + (((double)z + (1.5 - mod)) * dz));
                                //if (!Intersect_26Pt(Loc, out BDir, out abs, ref Rnd))
                                if (!Intersect_13Pt(Loc, out BDir, out abs, ref Rnd))
                                {
                ...
                public bool Intersect_13Pt(Point Center, out List<Bound_Node.Boundary> Bout, out List<double[]> alpha, ref Random Rnd)
                {
                    Bout = new List<Bound_Node.Boundary>();
                    alpha = new List<double[]>();
                ...
                public class RDD_Node: Node
                {
                    ...
                    public override void Link_Nodes(ref Node[][][] Frame, int x, int y, int z)
                    {
                        ...
                        int xdim = Frame.Length - 1;
                        int ydim = Frame[0].Length / 2 + (mod) - 1;
                        int zdim = Frame[0][0].Length / 2 + (mod) - 1;

                        Links2 = new RDD_Node[12];

                        if (mod == 0)
                        {
                            if (x < xdim && y < ydim) Links2[0] = Frame[x + 1][y][z-1]; else Links2[0] = new Null_Node();
                            if (x > 0 && y > 0) Links2[1] = Frame[x - 1][y-1][z-1]; else Links2[1] = new Null_Node();
                            if (x < xdim && y > 0) Links2[2] = Frame[x + 1][y-1][z-1]; else Links2[2] = new Null_Node();
                            if (x > 0 && y < ydim) Links2[3] = Frame[x - 1][y][z-1]; else Links2[3] = new Null_Node();
                            if (x < xdim && z < zdim) Links2[4] = Frame[x + 1][y-1][z]; else Links2[4] = new Null_Node();
                            if (x > 0 && z > 0) Links2[5] = Frame[x - 1][y-1][z-1]; else Links2[5] = new Null_Node();
                            if (x > 0 && z < zdim) Links2[6] = Frame[x - 1][y-1][z]; else Links2[6] = new Null_Node();
                            if (x < xdim && z > 0) Links2[7] = Frame[x + 1][y-1][z-1]; else Links2[7] = new Null_Node();
                            if (y < ydim && z < zdim) Links2[8] = Frame[x][y][z]; else Links2[8] = new Null_Node();
                            if (y > 0 && z > 0) Links2[9] = Frame[x][y-1][z-1]; else Links2[9] = new Null_Node();
                            if (y > 0 && z < zdim) Links2[10] = Frame[x][y-1][z]; else Links2[10] = new Null_Node();
                            if (y < ydim && z > 0) Links2[11] = Frame[x][y][z-1]; else Links2[11] = new Null_Node();
                        }
                        else
                        {
                    ...
                    {
                        double p2 = 0;
                        foreach (RDD_Node node in Links2) p2 += node.P;
                        Pnf = p2 * 0.25 - Pn - Pn_1;
                    }
                }
